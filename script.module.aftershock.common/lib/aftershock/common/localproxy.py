...
class MyHandler(BaseHTTPRequestHandler):
    ...
    """
    def do_HEAD(self):
        print "LocalProxy: Serving HEAD request..."
    ...
    """
    def do_GET(self):
        print "LocalProxy: Serving GET request..."
        self.answer_request(1)

    def answer_request(self, sendData):
        try:
            request_path = self.path[1:]
            print 'LocalProxy: original request_path: ' + request_path
            ...
            print 'LocalProxy: 2-request_path: ' + request_path
            if request_path == "stop":
                ...
                print 'LocalProxy: realpath: ' + realpath
                fURL = base64.b64decode(realpath)
                print 'LocalProxy: sendData %s realpath(decoded): %s' % (sendData,realpath)
        ...
        if (sendData):
            ...
            try:
                ...
                try:
                    ...
                    print time.asctime(), "LocalProxy: Client Closed the connection."
                    try:
                        response.close()
                        fileout.close()
                    except Exception, e:
...
if __name__ == '__main__':
    ...
    print "LocalProxy: Starts - %s:%s" % (HOST_NAME, PORT_NUMBER)
    httpd.serve_forever()
    #while(True):
    #    httpd.handle_request()
    httpd.server_close()
    print "LocalProxy: Stops %s:%s" % (HOST_NAME, PORT_NUMBER)

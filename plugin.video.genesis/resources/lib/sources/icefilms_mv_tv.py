...
class source:
    def __init__(self):
        ...
        self.headers = { 'Referer' : 'http://www.icefilms.info/',
    ...
    def get_show(self, imdb, tvdb, tvshowtitle, year):
        try:
            ...
            links = [self.link_1]
            for base_link in links:
                result = cloudflare.source(urlparse.urljoin(base_link, query), headers=self.headers)
                if 'Donate' in str(result): break
    ...
    def get_sources(self, url, hosthdDict, hostDict, locDict):
        try:
            ...
            if len(hd) == 0 : hd = [i for i in links if 'HD 720p' in i]
            ...
            if len(sd) == 0: sd = [i for i in links if 'DVD 480p' in i]

...
class source:
    ...
    def get_sources(self, url, hosthdDict, hostDict, locDict):
        try:
            ...
            result = [i[0] + i[1] for i in result if len(i[2]) > 0]
            result = [i[0] for i in result] # flatten the list

            for i in result:
                url = urlparse.urljoin(self.base_link, i)

                i = client.source(url, close=False)

                url = client.parseDOM(i, 'span', attrs = {'class': 'object-wrapper'})[0]
                url = client.parseDOM(url, 'iframe', ret='src')[0]
                url = client.replaceHTMLCodes(url)

                response = client.source(url, close=False)

                try :
                    r = re.compile("create_ifr\((.+?)',.+\);").findall(response)[0]
                    r = r.replace('\'','').replace('+','')
                    u = '%s|User-Agent=%s&Referer=%s' % ((r, urllib.quote_plus(client.agent()), urllib.quote_plus(sources_url)), i[0], 'Dizibox')
                    try: sources.append({'source': 'Ok', 'quality': 'SD', 'provider': 'Dizibox', 'url': u})
                    except: pass
                except : pass

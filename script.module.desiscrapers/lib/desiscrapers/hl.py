...
from aftershock.common import cleantitle, control
...
class HostedLink:
    def __init__(self, title, year, imdb=None, tvdb=None, host=None, include_disabled=False, timeout=30, exclude=None, enable_debrid=False, scraper_type=None):
        self.title = title
        self.year = year
        self.imdb = imdb
        self.tvdb = tvdb
        ...
        self.__scrapers = self.__get_scrapers(include_disabled, exclude, scraper_type)
        random.shuffle(self.__scrapers)
        path = control.dataPath
        control.makeFile(path)
        self.cache_location = control.cacheFile



    def __get_scrapers(self, include_disabled, exclude, scraper_type):
        klasses = desiscrapers.relevant_scrapers(self.host, include_disabled, exclude=exclude, scraper_type=scraper_type)
        scrapers = []
            ...
            try:
                ...
                dbcur.execute(
                    "SELECT * FROM rel_src WHERE scraper = '%s' AND title = '%s' AND show_year= '%s' AND year = '%s' AND season = '%s' AND episode = '%s'" % (
                        scraper.name, cleantitle.get(title).upper(), show_year, year, season, episode))
                match = dbcur.fetchone()
            ...
            if sources == None:
                ...
                if cache_enabled:
                    ...
                    dbcur.execute("INSERT INTO rel_src Values (?, ?, ?, ?, ?, ?, ?, ?, ?)", (
                        scraper.name, cleantitle.get(title).upper(), show_year, year, season, episode, imdb,
                        json.dumps(sources),
            ...
            try:
                ...
                dbcur.execute(
                    "SELECT * FROM rel_music_src WHERE scraper = '%s' AND title = '%s' AND artist = '%s'" % (
                        scraper.name, cleantitle.get(title).upper(), artist.upper()))
                match = dbcur.fetchone()
            ...
            if sources == None:
                ...
                if cache_enabled:
                    ...
                    dbcur.execute("INSERT INTO rel_music_src Values (?, ?, ?, ?, ?)", (
                        scraper.name, cleantitle.get(title).upper(), artist.upper(), json.dumps(sources),
                        datetime.datetime.now().strftime("%Y-%m-%d %H:%M")))
                    dbcon.commit()

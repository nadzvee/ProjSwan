...
class FanArtWindow(control.WindowDialog):
    def __init__(self):
        control_background = control.controlImage(0, 0, 1280, 720, control.addonInfo('fanart'))
...
class ExtendedDialogHacks(object):
    ...
    def background_task(self):
        control.sleep(1000)
        while not control.abortRequested and self.active:
            if self.hide_progress:
                active_window = control.currentWindowDialogId
                if active_window in [10101, 10151]:
                    control.execute("Dialog.Close(%d, true)" % active_window)
            if self.hide_info:
                if control.condVisibility("Window.IsActive(infodialog)"):
                    control.execute('Dialog.Close(infodialog, true)')
            control.sleep(100)
...
class SelectorDialog(control.windowXMLDialog):
    def __init__(self, *args, **kwargs):
        control.windowXMLDialog.__init__(self)
    ...
    def _populate(self):
        control.sleep(500)  # Delay population to let ui settle
        self.label.setLabel(self.title)

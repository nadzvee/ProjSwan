...
class WRMovies(Scraper):
    domains = ['watchrecentmovies.com']
    name = "wrmovies"

    def __init__(self):
        self.base_link = 'https://watchrecentmovies.com'
    ...
    def sources(self, url):
        ...
        try:
            ...
            items = client.parseDOM(result, "div", attrs={"class": "video-container"})
            items += client.parseDOM(result, "div", attrs={"class":"donwload_link hsg"})

            #items = client.parseDOM(result, "article")

            for item in items:
                try :
                    url = re.compile('(SRC|src|data-config|href)=[\'|\"](.+?)[\'|\"]').findall(item)[0][1]
                    host = client.host(url)
                    srcs.append({'source': host, 'parts' : '1', 'quality': 'SD', 'scraper': self.name, 'url': url, 'direct':False})
                except :
                    pass

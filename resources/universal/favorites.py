...
class Favorites:
    ...
    def delete_item(self, title, fmtd_title='', item_mode='main', section_title='Misc.', section_addon_title='', sub_section_title='', sub_section_addon_title=''):
        ...
        uni_fav_add_script = 'XBMC.RunScript(%s, %s, %s, "%s")' % (self._get_script_path(), self.sys_argv[1], self._build_params(uni_fav), 'plugin.video.movie25.favorites')
        
        return uni_fav_add_script
    
    def _get_script_path(self):
        return os.path.join('special://home/addons/plugin.video.movie25','resources','universal','favorites.py')
...
if sys.argv and len(sys.argv) >= 4 and sys.argv[3] == 'plugin.video.movie25.favorites':    
    
    sys.argv[0] = 'plugin.video.movie25'
    addon_fav = Addon('plugin.video.movie25', sys.argv)

...
if __name__ == '__main__':
    try:
        ADDON = xbmcaddon.Addon('script.aftershocknow.guide')
        if ADDON.getSetting('autostart') == "true":
            xbmc.executebuiltin("RunAddon(script.aftershocknow.guide)")
        
        if 'false' == 'true':
            monitor = xbmc.Monitor()
            xbmc.log("[script.aftershocknow.guide] Background service started...", xbmc.LOGDEBUG)
            Service()
            interval = 0
            ...
            while not monitor.abortRequested():
                ...
                xbmc.log("[script.aftershocknow.guide] Service waiting for interval %s" % waitTime, xbmc.LOGDEBUG)
                if monitor.waitForAbort(waitTime):
                    # Abort was requested while waiting. We should exit
                    break
                xbmc.log("[script.aftershocknow.guide] Service now triggered...", xbmc.LOGDEBUG)
                Service()
                
    except source.SourceNotConfiguredException:
        pass  # ignore
    except Exception, ex:
        xbmc.log('[script.aftershocknow.guide] Uncaught exception in service.py: %s' % str(ex), xbmc.LOGDEBUG)

...
class Program(object):
    def __init__(self, channel, title, startDate, endDate, description, rec_url, set_url, media_type, scheduleid, imageLarge=None, imageSmall=None,
    ...
    def __repr__(self):
        return 'Program(channel=%s, title=%s, startDate=%s, endDate=%s, description=%s, rec_url=%s, set_url=%s, media_type=%s, scheduleid=%s, imageLarge=%s, ' \
               'imageSmall=%s, episode=%s, season=%s, is_movie=%s)' % (self.channel, self.title, self.startDate,
                                                                       self.endDate, self.description, self.rec_url, self.set_url, self.media_type, self.scheduleid, self.imageLarge, self.imageSmall, self.season, self.episode, self.is_movie)
...
class Database(object):
        ...
        while True:
            ...
            try:
                ...
                if cancel_requested_callback is None:
                    xbmc.log('[script.aftershocknow.guide] Database is locked, bailing out...', xbmc.LOGDEBUG)
                    break
                else:  # ignore 'database is locked'
                    xbmc.log('[script.aftershocknow.guide] Database is locked, retrying...', xbmc.LOGDEBUG)

            except sqlite3.DatabaseError:
                self.conn = None
                if self.alreadyTriedUnlinking:
                    xbmc.log('[script.aftershocknow.guide] Database is broken and unlink() failed', xbmc.LOGDEBUG)
                    break
                else:
                    try:
        ...
        try:
            ...
            for item in self.source.getDataFromExternal(date, progress_callback):
                ...
                if isinstance(item, Channel):
                    ...
                    c.execute(
                        'INSERT INTO programs(channel, title, start_date, end_date, description, rec_url, set_url, media_type, scheduleid, image_large, image_small, season, episode, is_movie, language, source, updates_id) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
                        [channel, program.title, program.startDate, program.endDate, program.description, program.rec_url, program.set_url, program.media_type, program.scheduleid, program.imageLarge, program.imageSmall, program.season, program.episode, program.is_movie, program.language, self.source.KEY, updatesId])
    ...
    def _getCurrentProgram(self, channel):
        ...
        row = c.fetchone()
        if row:
            program = Program(channel, row['title'], row['start_date'], row['end_date'], row['description'], row['rec_url'], row['set_url'], row['media_type'], row['scheduleid'], row['image_large'], row['image_small'], None, row['season'], row['episode'], row['is_movie'], row['language'])
        c.close()
    ...
    def _getNextProgram(self, program):
        ...
        row = c.fetchone()
        if row:
            nextProgram = Program(program.channel, row['title'], row['start_date'], row['end_date'], row['description'], row['rec_url'], row['set_url'], row['media_type'], row['scheduleid'], row['image_large'], row['image_small'], None, row['season'], row['episode'], row['is_movie'], row['language'])
        c.close()
    ...
    def _getPreviousProgram(self, program):
        ...
        if row:
            previousProgram = Program(program.channel, row['title'], row['start_date'], row['end_date'],
                                      row['description'], row['rec_url'], row['set_url'], row['media_type'], row['scheduleid'], row['image_large'], row['image_small'], None, row['season'],
    ...
    def _getProgramList(self, channels, startTime):
        ...
        c = self.conn.cursor()
        c.execute(
            'SELECT p.*, (SELECT 1 FROM notifications n WHERE n.channel=p.channel AND n.program_title=p.scheduleid AND n.source=p.source) AS notification_scheduled FROM programs p WHERE p.channel IN (\'' + (
    ...
    def _createTables(self):
        ...
        try:
            if version < [1, 3, 1]:
                ...
                c.execute('INSERT INTO version(major, minor, patch) VALUES(1, 3, 1)')
                # For caching data
                ...
                c.execute(
                    'CREATE TABLE programs(channel TEXT, title TEXT, start_date TIMESTAMP, end_date TIMESTAMP, description TEXT, rec_url TEXT, set_url TEXT, media_type TEXT, scheduleid TEXT, image_large TEXT, image_small TEXT, source TEXT, updates_id INTEGER, FOREIGN KEY(channel, source) REFERENCES channels(id, source) ON DELETE CASCADE, FOREIGN KEY(updates_id) REFERENCES updates(id) ON DELETE CASCADE)')
            ...
            if version < [1, 3, 2]:
                ...
                c.execute(
                    'CREATE TABLE programs(channel TEXT, title TEXT, start_date TIMESTAMP, end_date TIMESTAMP, description TEXT, rec_url TEXT, set_url TEXT, media_type TEXT, scheduleid TEXT, image_large TEXT, image_small TEXT, source TEXT, updates_id INTEGER, FOREIGN KEY(channel, source) REFERENCES channels(id, source) ON DELETE CASCADE DEFERRABLE INITIALLY DEFERRED, FOREIGN KEY(updates_id) REFERENCES updates(id) ON DELETE CASCADE DEFERRABLE INITIALLY DEFERRED)')
                c.execute('CREATE INDEX program_list_idx ON programs(source, channel, start_date, end_date)')
                c.execute('CREATE INDEX start_date_idx ON programs(start_date)')
                c.execute('CREATE INDEX end_date_idx ON programs(end_date)')
            ...
            if version < [1, 3, 3]:
                ...
                c.execute('DROP TABLE programs')
                c.execute(
                    'CREATE TABLE programs(channel TEXT, title TEXT, start_date TIMESTAMP, end_date TIMESTAMP, description TEXT, rec_url TEXT, set_url TEXT, media_type TEXT, scheduleid TEXT, image_large TEXT, image_small TEXT, season TEXT, episode TEXT, is_movie TEXT, language TEXT, source TEXT, updates_id INTEGER, FOREIGN KEY(channel, source) REFERENCES channels(id, source) ON DELETE CASCADE DEFERRABLE INITIALLY DEFERRED, FOREIGN KEY(updates_id) REFERENCES updates(id) ON DELETE CASCADE DEFERRABLE INITIALLY DEFERRED)')
                c.execute('CREATE INDEX program_list_idx ON programs(source, channel, start_date, end_date)')
                c.execute('CREATE INDEX start_date_idx ON programs(start_date)')
                c.execute('CREATE INDEX end_date_idx ON programs(end_date)')
    ...
    def _addNotification(self, program):
        ...
        c.execute("INSERT INTO notifications(channel, program_title, source) VALUES(?, ?, ?)",
                  [program.channel.id, program.scheduleid, self.source.KEY])
        self.conn.commit()
        c.close()
    ...
    def _getNotifications(self, daysLimit):
        ...
        c = self.conn.cursor()
        c.execute(
            "SELECT DISTINCT c.title, p.title, p.start_date FROM notifications n, channels c, programs p WHERE n.channel = c.id AND p.channel = c.id AND n.scheduleid = p.title AND n.source=? AND p.start_date >= ? AND p.end_date <= ?",
...
class XMLTVSource(Source):
    PLUGIN_DATA = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.aftershocknow.guide'))
    KEY = 'xmltv'
    INI_TYPE_FTV = 0
    INI_TYPE_CUSTOM = 1
    INI_FILE = 'addons.ini'
    ...
    def parseXMLTVDate(self, origDateString):
        if origDateString.find(' ') != -1:
            ...
            try:
                ...
                xbmc.log('[script.aftershocknow.guide] strptime error with this date: %s' % dateString, xbmc.LOGDEBUG)
        ...
        for event, elem in context:
            if event == "end":
                ...
                if elem.tag == "programme":
                    ...
                    rec_url = elem.findtext("rec_url")
                    set_url = elem.findtext("set_url")
                    media_type = elem.findtext("media_type")
                    scheduleid = elem.findtext("scheduleid")
                    ...
                    result = Program(channel, elem.findtext('title'), self.parseXMLTVDate(elem.get('start')),
                                     self.parseXMLTVDate(elem.get('stop')), description, rec_url, set_url, media_type, scheduleid, imageSmall=icon,

...
from aftershock.common import control, cleantitle, client, cache, metacache, workers, views, logger
...
class tvshows:
    ...
    def super_info(self, i):
        try :
            ...
            url = self.burp_search_link % urllib.quote_plus(title.encode('utf-8'), safe=':/')
            ...
            for item in result:
                linkTitle = client.parseDOM(item, name="a", attrs={"class": "title"})[0]
    ...
    def tvshowDirectory(self, items, confViewMode='list', estViewMode='widelist'):
        ...
        for i in items:
            try:
                label = i['name']
                systitle = sysname = urllib.quote_plus(i['title'].encode('utf-8'), safe=':/')
